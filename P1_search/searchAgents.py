...
from markdown_it.common.html_re import close_tag
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    # At first design, it only calculates the length to each corner and picks a minimum heuristics.
    # When it reaches one of the corner, delete the corner and recalculate length to the rest.
    # But it cannot complete the task within 1000 steps.
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    """
    # In this section, we cannot use the previous answer anymore, since there exists a case that
    # all foods lie in the line so that, in previous one, we sum up length from start to each goal
    # which overestimates the real cost.
    position, foodGrid = state
    food_grid = foodGrid.asList()
    if not food_grid:
        return 0
    # In order to expend fewer nodes, we choose to reach the furthest node using max so that
    # we may go through some nodes in the process.
    # mazeDistance actually is not a heuristic function as it calculates the real distance between two nodes.
