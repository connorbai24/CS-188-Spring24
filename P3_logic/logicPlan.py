...
def sentence1() -> Expr:
    ...
    A_or_B = Expr('A') | Expr('B')
    secClause = ~ Expr('A') % (~Expr('B') | Expr('C'))
    thirdClause = disjoin([~Expr('A'), ~Expr('B'), Expr('C')])
    return conjoin([A_or_B, secClause, thirdClause])
...
def sentence2() -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    firstClause = Expr('C') % (Expr('B') | Expr('D'))
    secondClause = Expr('A') >> (~Expr('B') & ~Expr('D'))
    thirdClause = ~ (Expr('B') & ~ Expr('C')) >> Expr('A')
    forthClause = ~ Expr('D') >> Expr('C')
    return conjoin([firstClause, secondClause, thirdClause, forthClause])
...
def sentence3() -> Expr:
    ...
    firstClause = PacmanAlive_1 % ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0))
    secondClause = ~ (PacmanAlive_0 & PacmanBorn_0)
    thirdClause = PacmanBorn_0
    return conjoin([firstClause, secondClause, thirdClause])
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    unsatisfiability = premise & ~conclusion
    if not findModel(unsatisfiability):
        return True
    else:
        return False
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None,
    ...
    pacphysics_sentences.append(exactlyOne(onePlace))
    oneAction = []
    for action in DIRECTIONS:
        oneAction.append(PropSymbolExpr(action, time=t))
    pacphysics_sentences.append(exactlyOne(oneAction))

    if sensorModel:
        pacphysics_sentences.append(sensorModel(t, non_outer_wall_coords))
    if successorAxioms and t != 0: # from pacmanSuccessorAxiomsSingle, we know it uses t-1.
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for (x, y) in non_wall_coords:
            onePlace.append(PropSymbolExpr(pacman_str, x, y, time=t))
        KB.append(exactlyOne(onePlace))
        reachGoal = findModel(conjoin(KB) & conjoin([~PropSymbolExpr(food_str, x, y, time=t) for x, y in food]))
        if reachGoal:
            return extractActionSequence(reachGoal, actions)
        oneAction = []
        for action in DIRECTIONS:
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        KB.append(fourBitPerceptRules(t, agent.getPercepts()))
        for x, y in non_outer_wall_coords:
            wallExpr = PropSymbolExpr(wall_str, x, y)
            if entails(conjoin(KB), wallExpr):
                KB.append(wallExpr)
                known_map[x][y] = 1 # update a known map
            if entails(conjoin(KB), ~wallExpr):
                KB.append(~wallExpr)
                known_map[x][y] = 0
        agent.moveToNextState(agent.actions[t])
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB.append(
            pacphysicsAxioms(t, all_coords, non_outer_wall_coords, known_map, SLAMSensorAxioms, SLAMSuccessorAxioms))
        KB.append(PropSymbolExpr(agent.actions[t], time=t))
...
def SLAMSensorAxioms(t: int, non_outer_wall_coords: List[Tuple[int, int]]) -> Expr:
    ...
    for direction in DIRECTIONS:
        ...
        for x, y in non_outer_wall_coords:
            combo_var = PropSymbolExpr(pacman_wall_str, x, y, x + dx, y + dy, time=t)
            percept_exprs.append(combo_var)
            combo_var_def_exprs.append(combo_var % (PropSymbolExpr(pacman_str, x, y, time=t)
